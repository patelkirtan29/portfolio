...
// The actual 3D content behind a Work-grid trinket — Canvas, meshes, and
// pointer-driven toy physics. Split out of Trinket.tsx so that file can stay
// a tiny, always-server/client-safe wrapper: Trinket.tsx dynamically imports
// *this* module client-only (`next/dynamic(..., { ssr: false })`, same
// pattern hero/HeroSceneLoader.tsx uses for DeskCanvas), which keeps the
// ~1.1MB three.js/@react-three/fiber bundle out of every route's initial
// HTML shell — it previously loaded eagerly on /work because ProjectCard.tsx
// imported the Canvas-bearing component as a plain top-level import.
//
...
//
// Color management: explicit ACES Filmic tone mapping + sRGB output color
// space on the `gl` prop below, matching the hero's `DeskCanvas.tsx` — kept
// in sync so the two 3D layers on the site (hero desk scene, Work trinkets)
// read with consistent color/contrast treatment rather than one using
// three.js's flatter linear defaults.
...
export type TrinketCanvasProps = {
  project: Project;
...
export default function TrinketCanvas({ project, size = 64 }: TrinketCanvasProps) {
